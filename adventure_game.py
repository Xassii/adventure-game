...
def def_int_input(min, max, text=''):
    ...
    text : str, optional
        Text to describe what the user can choose and what the choises mean.
    
    Returns
    -------
    int
        The user choosen value.
    """
    if not text:
...
def connect_tiles(tiles):
    for room in tiles:
        for other_room in tiles:
            ...
            exist_con = room.doors.keys()
            
            if x2 + 1 == x1 and y2 == y1 and not 'South' in exist_con:
                room.connect(other_room, 'South', 'North')
                if random.randint(0, 9):
                    break
            elif x2 - 1 == x1 and y2 == y1 and not 'North' in exist_con:
                room.connect(other_room, 'North', 'South')
                if random.randint(0, 9):
                    break
            elif x2 == x1 and y2 + 1 == y1 and not 'West' in exist_con:
                room.connect(other_room, 'West', 'East')
                if random.randint(0, 9):
                    break
            elif x2 == x1 and y2 - 1 == y1 and not 'East' in exist_con:
...
def create_tiles(): #This aint great
    ...
    tiles[0].type = 'S'
    for i in types:
        while True: #TODO break into small function
...
def play(): #TODO To long. Break into smaler functions
    game = setupp()
    encounter = False
    print(f'\n\n----------- oOo -----------')
    print(at.start)
